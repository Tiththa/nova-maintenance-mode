<template>
    <div>
        <div class="flex mb-6 items-center justify-between">
            <heading>{{ __('Maintenance Mode') }}</heading>
        ...
        <card  class="mb-6 py-3 px-6">
            ...
            <div v-if="currentlyInMaintenance" class="mb-6 flex flex-col items-center justify-center">
                ...
                <span>{{ __('You are currently in Maintenance Mode') }}</span>
            </div>
            </transition>
            ...
            <div v-if="!currentlyInMaintenance">
                <div class="flex border-b border-40">
                    ...
                    <div class="w-3/4 py-4">
                        <input v-model="message" type="text" name="message" class="form-control form-input form-input-bordered w-1/2 " placeholder="A message for your users"> 
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4"><h4 class="font-normal text-80">{{ __('Retry Time') }} <span class="text-sm">(secs)</span></h4></div>
                    <div class="w-3/4 py-4">
                        <input v-model="retry" type="number" name="retry" class="form-control form-input form-input-bordered w-1/4" placeholder="Set the Retry-After header"  min=0 max=9999> 
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4"><h4 class="font-normal text-80">{{ __('Allowed IP Addresses') }}</h4></div>
                    <div class="w-3/4 py-4">
                        <input v-model="allow" type="text" name="allow" class="form-control form-input form-input-bordered w-1/2" placeholder="127.0.0.1, 192.168.0.1">
                    </div>
                </div>
                <div class="flex">
                    <div class="w-1/4 py-4"><h4 class="font-normal text-80">{{ __('Include your current IP') }}</h4></div>
                    <div class="w-3/4 py-4">
                        <toggle-button v-model="include_current_ip" class="mb-4" width="50" height="22" speed="400"/>
                        <p class="text-danger">NOTE</p>
                        <p class="text-80">{{ __('If you uncheck this and do not add your IP address above you will lose access to Nova as well') }}</p>
                    </div>
                </div>
            </div>
